...
class Settings(BaseSettings):
    ...
    # Position sizing risks ``risk_per_trade_pct`` of ``account_size`` per trade.
    # Stop-loss = ATR * atr_stop_multiplier; target placed at target_rr * risk;
    # a plan is rejected when its achievable risk-reward < min_risk_reward.
    account_size: float = Field(default=1_000_000.0)
    risk_per_trade_pct: float = Field(default=0.01)   # 1% of account per trade
    min_risk_reward: float = Field(default=2.0)       # enforced 1:2 minimum
    atr_stop_multiplier: float = Field(default=1.5)
    target_rr: float = Field(default=2.0)
