...
No buy/sell or risk logic — direction, score, trend, confidence only. The last
close is carried through (set by the engine) so downstream consumers — e.g. the
Phase 4 risk engine — can read the entry price without a second DB round-trip.
"""
...
class TechnicalResult:
    ...
    n_candles: int
    insufficient: bool
    close: float | None = None   # last close (set by engine; entry source)
