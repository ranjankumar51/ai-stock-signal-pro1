...
not generate signals — only indicator values, trend, score, confidence, and the
last close (used downstream as the risk-engine entry price).
...
class TechnicalAnalysisEngine:
    ...
    async def analyze(self, instrument_id: int, tf: Timeframe) -> TechnicalResult:
        ...
        result.close = float(series.close[-1]) if len(series) else None
        return result
