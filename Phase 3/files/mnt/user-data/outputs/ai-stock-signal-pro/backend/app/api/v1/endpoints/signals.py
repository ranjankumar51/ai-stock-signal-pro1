...
Generation is synchronous and broker-free: the fusion engine reads only from
PostgreSQL (price_data via the point-in-time DataFeed), so no job queue is
...
async def generate_signal(
    ...
    return SignalResponse(
        ...
        status=gen.classification.status.value,
        weights_version=gen.fusion.weights_version,
        snapshot_id=gen.snapshot_id,
        rationale=gen.fusion.to_rationale(tf.value),
        mode=gen.mode.value,
    )
