...
class SnapshotRepository:
    ...
    async def upsert(
        ...
        *,
        ...
        """Insert or update the snapshot for (instrument, tf, ts) and return its id.

        Only the technical fields are written in Phase 2/3; the other dimension
        scores remain NULL until their engines exist. The returned id lets the
        signal-generation pipeline link the signal to its source snapshot.
        """
